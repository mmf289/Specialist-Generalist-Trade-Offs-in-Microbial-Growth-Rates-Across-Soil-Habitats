...
###NRI NTI analysis
tree<-read.newick("/Users/mfoley2/Library/CloudStorage/OneDrive-UniversitédeLausanne/github/DRIPITS_seqs_2x2.fa.aln.tre")
...
NRI <- as.matrix(-1 * ((ses.mpd.out[,2] - ses.mpd.out[,3]) / ses.mpd.out[,4]))

rownames(NRI) <- row.names(ses.mpd.out)
colnames(NRI) <- "NRI"
