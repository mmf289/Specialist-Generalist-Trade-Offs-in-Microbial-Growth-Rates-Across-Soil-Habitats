...
###create comparisons
#194, 196, 198, 200
rhizo_normal_194 <- run_feature_filter(qsip_object,
                                           unlabeled_source_mat_ids = c(193, 195, 197, 199),
                                           labeled_source_mat_ids = 194,
...
rhizo_normal_196 <- run_feature_filter(qsip_object,
                                       unlabeled_source_mat_ids = c(193, 195, 197, 199),
                                       labeled_source_mat_ids = 196,
                                       min_unlabeled_sources = 2,
                                       min_labeled_sources = 1,
                                       min_unlabeled_fractions = 2,
                                       min_labeled_fractions = 2
...
rhizo_normal_198 <- run_feature_filter(qsip_object,
                                       unlabeled_source_mat_ids = c(193, 195, 197, 199),
                                       labeled_source_mat_ids = 198,
                                       min_unlabeled_sources = 2,
                                       min_labeled_sources = 1,
                                       min_unlabeled_fractions = 2,
                                       min_labeled_fractions = 2
...
rhizo_normal_200 <- run_feature_filter(qsip_object,
                                       unlabeled_source_mat_ids = c(193, 195, 197, 199),
                                       labeled_source_mat_ids = 200,
                                       min_unlabeled_sources = 2,
                                       min_labeled_sources = 1,
                                       min_unlabeled_fractions = 2,
                                       min_labeled_fractions = 2
...
##reasmpling
#194, 196, 198, 200
rhizo_normal_194 <- run_resampling(rhizo_normal_194,
                                       resamples = 1000,
                                       with_seed = 17,
                                       progress = FALSE,
                                       allow_failures = T,
...
rhizo_normal_196 <- run_resampling(rhizo_normal_196,
                                   resamples = 1000,
                                   with_seed = 17,
                                   progress = FALSE,
                                   allow_failures = T,
...
rhizo_normal_198 <- run_resampling(rhizo_normal_198,
                                   resamples = 1000,
                                   with_seed = 17,
                                   progress = FALSE,
                                   allow_failures = T,
...
rhizo_normal_200 <- run_resampling(rhizo_normal_200,
                                   resamples = 1000,
                                   with_seed = 17,
                                   progress = FALSE,
                                   allow_failures = T,
...
## calc eaf
rhizo_normal_194 <- run_EAF_calculations(rhizo_normal_194)
rhizo_normal_196 <- run_EAF_calculations(rhizo_normal_196)
rhizo_normal_198 <- run_EAF_calculations(rhizo_normal_198)
rhizo_normal_200 <- run_EAF_calculations(rhizo_normal_200)


r_n_194 <- summarize_EAF_values(rhizo_normal_194, confidence = 0.9) %>% mutate(tube = "194")
r_n_196 <- summarize_EAF_values(rhizo_normal_196, confidence = 0.9) %>% mutate(tube = "196")
r_n_198 <- summarize_EAF_values(rhizo_normal_198, confidence = 0.9) %>% mutate(tube = "198")
r_n_200 <- summarize_EAF_values(rhizo_normal_200, confidence = 0.9) %>% mutate(tube = "200")

all.data = rbind(r_n_194, r_n_196)
all.data.2 = rbind(all.data, r_n_198)
all.data.3 = rbind(all.data.2, r_n_200)

ggplot(all.data.3) + 
  geom_boxplot(pch = 21, color = "black", aes(x=as.factor(tube), y=observed_EAF))

write.csv(all.data.3, "~/Desktop/H3_qsip_redo/jeff code/16S/rhizo_normal.csv")
